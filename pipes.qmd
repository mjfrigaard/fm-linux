...
**`echo "<SOMETHING>" | wc -w`** sends the output of the <code>@sec-echo</code> command to <code>@sec-wc</code>, which then counts the words.
...
**<code>@sec-cat</code> `<FILE_PATH> |` <code>@sec-grep</code> `<PATTERN>`** prints the contents from `<FILE_PATH>` that contain the `<PATTERN>`.
...
**<code>@sec-ls</code>` -l <FILE_PATH> | `<code>@sec-sort</code>` -r`** lists the files in `<FILE_PATH>` in a detailed format, then sorts them in reverse order.

```{bash}
ls -l data/raw | sort -r
```

`-r` showcases how to reverse the listing of directory contents.
...
# Jun 17 16:29:49 Martins-MacBook-Pro-2 login[866]: DEAD_PROCESS: 866 ttys000
# Jun 17 16:29:49 Martins-MacBook-Pro-2 login[1159]: DEAD_PROCESS: 1159 ttys001
# Jun 17 16:29:49 Martins-MacBook-Pro-2 login[1392]: DEAD_PROCESS: 1392 ttys002
# Jun 17 16:29:52 Martins-MacBook-Pro-2 sessionlogoutd[11382]: DEAD_PROCESS: 196 console
```
...
**<code>@sec-grep</code>` "RZA" "$fname"`**: Searches for a specific pattern within the file. <code>@sec-grep</code> looks through the contents of the file (whose path is in `fname`) for lines containing the string "RZA". Only the lines that match this pattern are printed to `stdout`:
...
**<code>@sec-wc</code>**: For each file processed by the loop, <code>@sec-wc</code> outputs three numbers: 

1. The line count  
2. The word count, and  
3. The character/byte count of the lines that <code>@sec-grep</code> found to contain "RZA". 

**Example:**

If no options are given to <code>@sec-wc</code>, it defaults to displaying all three counts.
