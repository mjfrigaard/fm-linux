...
# [Permissions]{style="color: #9b2e0c;"} {#sec-perms}
...
**Example:**
To give the owner full access, the group read/execute, and others none:
...
1. **Setuid (s on owner’s x)**
   When set on an executable file, this lets users run it with the file owner’s permissions.
2. **Setgid (s on group’s x)**
   Similar to setuid, but for group permissions; on folders, new files inherit the folder’s group.
3. **Sticky bit (t on others’ x)**
   On a folder, users can only delete or rename files they own, even if others have write permission.
